...
/// 从一个id类型中 解析成NSNumber
/// Parse a number value from 'id'.
static force_inline NSNumber *YYNSNumberCreateFromID(__unsafe_unretained id value) {
    
//   NSCharacterSet ｜ http://blog.csdn.net/lcl130/article/details/41802623
...
//            将字符串转换为double(双精度浮点数) ｜ http://c.biancheng.net/cpp/html/124.html
            
            double num = atof(cstring);
//            如果为非数值 或者 无穷大的值 则返回nil
            if (isnan(num) || isinf(num)) return nil;
            return @(num);
        } else {
            const char *cstring = ((NSString *)value).UTF8String;
            if (!cstring) return nil;
//            atoll函数：将字符串转化为long long类型变量 | http://my.oschina.net/Tsybius2014/blog/338234
